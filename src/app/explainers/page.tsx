  ...
  },
  {
    ...
    a: (
      <>
        ...
        <strong>strong</strong> (government reports, peer-reviewed academic work, third-party credit
        ratings), <strong>moderate</strong> (program documentation, regulator market snapshots,
        news), and <strong>weak</strong> (single-source or contested claims). A project can have
        very strong fundamentals and still get a moderate evidence-confidence reading if much of its
        inner workings (financing terms, IBA contents, equity-option specifics) is not in the public
        record.
      </>
    ),
  },
  ...
  },
  {
    ...
    a: (
      <>
        ...
        and passed implementing legislation (the UN Declaration Act, Bill C-15) in 2021; the federal
        2023–2028 Action Plan commits to developing FPIC-aligned guidance for natural-resource
        project engagement (Measure 32). BC&apos;s DRIPA (2019) implements UNDRIP at the provincial
        level. FPIC is a higher standard than the &quot;duty to consult&quot; established in Haida
        (2004) — but the domestic legal effect of FPIC in Canadian project decisions is still being
  ...
  },
  {
    ...
    a: (
      <>
        ...
        statute. The Tsuu T&apos;ina Nation v. Alberta (2008 ABQB 547) decision held that Treaties 6
        and 7 do not establish a standalone right to water (though the door was left open for water
        rights ancillary to other treaty rights). This silence is itself a Treaty-Lab finding.
      </>
    ),
  },
  {
    ...
    a: (
      <>
        ...
        require. Site C and TMX are older Crown-financed projects where the equivalent specifics are
        buried in BC Hydro and Trans Mountain Corporation annual reports, court filings, and
  ...
  },
  {
    ...
    a: (
      <>
        ...
        common Canadian patterns: <strong>Cedar LNG</strong> (Indigenous-majority equity,
        co-development from inception), <strong>Coastal GasLink</strong> (private pipeline with
        partial Indigenous equity options, documented forfeiture case), <strong>Site C</strong>{" "}
        (provincial Crown hydroelectric, post-Yahey accommodation), and <strong>TMX</strong>{" "}
        (federal Crown acquisition with potential Indigenous divestment). Real production use would
        replace these with the projects each community or analyst actually cares about — the
        assessment template is the asset, not the pilot project list.
      </>
    ),
  },
  {
    ...
    a: (
      <>
        It is a <em>qualitative orientation tool</em>, not a market index. For each domain (treaty /
        water / energy / finance / governance), Treaty-Lab averages the severity rank of the
        indicators tagged to that domain (1 = low, 2 = moderate, 3 = elevated, 4 = high, 5 =
        critical) and plots the result on a radar. Higher score = more pressing exposure in the
  ...
  },
  {
    ...
    a: (
      <>
        ...
        texts appear in both, with Archive holding the longer text and Library holding the annotated
        supports / limitations metadata.
      </>
    ),
  },
];
        ...
        <Accordion className="mt-6">
          {FAQ_METHODOLOGY.map((item, i) => (
            <AccordionItem key={i} value={`faq-${i}`}>
              ...
              <AccordionContent>
                <div className="text-sm leading-relaxed text-foreground/85 max-w-3xl">{item.a}</div>
              </AccordionContent>
            </AccordionItem>
          ))}
        </Accordion>
